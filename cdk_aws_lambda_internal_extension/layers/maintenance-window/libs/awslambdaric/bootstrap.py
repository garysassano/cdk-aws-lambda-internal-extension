...
from .lambda_pre_handler import pre_handler_function
...
def run(app_root, handler, lambda_runtime_api_addr):
    ...
    with create_log_sink() as log_sink:
        ...
        while True:
            ...
            # START CUSTOM LOGIC
            try:
                pre_handler_function(event_request)
            except UserWarning as warning:
                error_result = build_fault_result(sys.exc_info(), None)
                xray_fault = make_xray_fault(
                    "LambdaUserWarning", warning.args[0], os.getcwd(), []
                )

                lambda_runtime_client.post_invocation_error(
                    event_request.invoke_id, to_json(error_result), to_json(xray_fault)
                )
